...
class ClassificationHead(nn.Module):
    def __init__(self, in_dim: int, n_classes: int, out_layer: nn.Module = nn.Identity()):
        super().__init__()
...
class MulticlassClassificationHead(ClassificationHead):
    def __init__(self, in_dim: int, n_classes: int):
        super().__init__(in_dim, n_classes, out_layer=nn.LogSoftmax(dim=1))


class MultilabelClassificationHead(ClassificationHead):
    def __init__(self, in_dim: int, n_classes: int):
        super().__init__(in_dim, n_classes, out_layer=nn.Sigmoid())
